...
async fn fft_mcl_sync(env: &mcl_rs::Mcl, sources: &mut Vec<Vec<Complex<f32>>>, results: &mut Vec<Vec<Complex<f32>>>) {
...
async fn fft_mcl_async(env: &mcl_rs::Mcl, sources: &mut Vec<Vec<Complex<f32>>>, results: &mut Vec<Vec<Complex<f32>>>) {
    let size = sources[0].len();
    let pes: [u64; 3] = [(size/2)  as u64, 1, 1];

    let num_iters: usize = (size as f64).log2().floor() as usize;
    
    let futures = sources.iter_mut().zip(results.iter_mut()).map(|(s,r)| {
        async move {
            for k in 0..num_iters{
                let p: i32 = 1 << k;
                if k == num_iters-1{
...
fn valid_result(result: &Vec<Complex<f32>>, reference: &Vec<Complex<f32>>) -> bool {
    result.iter().zip(reference.iter()).fold(true,|valid,(x,y)| {
...
fn fft() {
    ...
    let env = mcl_rs::MclEnvBuilder::new()
        .num_workers(workers)
        .initialize();
    env.create_prog("tests/fft.cl", mcl_rs::PrgType::Src).with_compile_args("-DSINGLE_PRECISION").load();
    ...
    for exp in 1..21{
        ...
        assert!(valid_result(&results[0],&reference),"SYNC fails for {dft_size}");
        ...
        assert!(valid_result(&results[0],&reference),"ASYNC fails for {dft_size}");
