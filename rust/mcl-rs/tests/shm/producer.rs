...
pub fn start( size: usize, iterations: usize){
    ...
    let shared_data = env.create_shared_buffer(mcl_rs::TaskArg::inout_shared::<u32>("mcl_shm_test",size*iterations));

    let hdls = (0..iterations).map(|i| start_iteration(&env,shared_data.sub_buffer(i*size..(i+1)*size))).collect::<Vec<_>>();
