...
static inline uint64_t __get_pes(mcl_device_t* dev)
{
    ...
    switch(dev->type) {
        case MCL_TASK_GPU:
            multiplier = MCL_DEV_MUL_GPU;
            break;
        case MCL_TASK_CPU:
            multiplier = MCL_DEV_MUL_CPU;
            break;
        case MCL_TASK_FPGA:
            multiplier = MCL_DEV_MUL_FPGA;
            break;
        default:
            break;
    }
    uint64_t pes = dev->punits * dev->wgsize * multiplier;  
	return pes;
}
	...
	*plt = (mcl_platform_t*) malloc(nplt * sizeof(mcl_platform_t));
	...
	for(i=0; i<nplt; i++){
		...
		for(j=0; j<p[i].ndev; j++){
			...
			Dprintf("    Platform %"PRIu64" Device %"PRIu64": Name: %s Type: 0x%"
				PRIx64" PEs: %"PRIu64" GlobalMemory: %f MB", 
...
int resource_create_ctxt(mcl_resource_t* res, uint64_t n)
{
	...
	for(i=0; i<n; i++){
		...
		for(j=0; j < nqueues; j++){
			...
			res[i].dev->cl_queue[j] = clCreateCommandQueueWithProperties(res[i].dev->cl_ctxt, res[i].dev->cl_dev,  NULL, &error);
			if(error != CL_SUCCESS){
				eprintf("Error creating command queue for MCL resource %"PRIu64"",i);
				goto err;
			}
#else
			res[i].dev->cl_queue[j] = clCreateCommandQueue(res[i].dev->cl_ctxt, res[i].dev->cl_dev, 0x0, &error);
			if(error != CL_SUCCESS){
				eprintf("Error creating command queue for MCL resource %"PRIu64"",i);
				goto err;
			}
