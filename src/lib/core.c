...
int __am_exec(mcl_handle *hdl, mcl_request *req, uint64_t flags)
{
    ...
    */
    ...
    swap_success = cas(&(hdl->status), MCL_REQ_ALLOCATED, MCL_REQ_PENDING);
    assert(swap_success);
...
static inline int __task_setup(mcl_request *r)
{
    ...
    if (r->hdl->cmd != MSG_CMD_TRAN)
    {
        ...
        else if (t->lpes[0] * t->lpes[1] * t->lpes[2] > kwgroup)
        {
...
static inline int cli_exec_am(struct worker_struct *desc, struct mcl_msg_struct *msg)
{
    ...
    for (i = 0; i < t->nargs; i++)
    {
        ...
        {
            Dprintf("\tEnqueing read buffer for argument: %d.", i);
...
err_req:
    ...
    swap_success = cas(&(h->status), MCL_REQ_INPROGRESS, MCL_REQ_COMPLETED);
    assert(swap_success);
    h->ret = MCL_RET_ERROR;
...
static inline int __check_task(mcl_request *r)
{
    ...
    if (eventStatus != CL_COMPLETE)
        return 1;
    uint8_t swap_success = cas(&(h->status), MCL_REQ_INPROGRESS, MCL_REQ_FINISHING);
    assert(swap_success);
...
void *check_pending(void *data)
{
    ...
    while (status != MCL_DONE || rlist_count(&ftasks, &ftasks_lock) > 0)
    {
        li = NULL;
        r = NULL;
